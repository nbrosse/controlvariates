...
#np.savez('Hb_erf.npz', H=H, H_zv=H_zv, b=b, b_zv=b_zv)

npzfile = np.load('Hb_erf.npz')
H = npzfile['H']
H_zv = npzfile['H_zv']
b = npzfile['b']
b_zv = npzfile['b_zv']
...
theta = np.linalg.lstsq(H, b, rcond=10**(-7))[0]
...
#    approx_dypois_mesh += theta[i] * dy_psi_mesh[i]  
  for i in np.arange(2*p**2):
    approx_dxpois_mesh += coeffs_pois[i] * dx_psi_mesh[i]  
    approx_dypois_mesh += coeffs_pois[i] * dy_psi_mesh[i]  
